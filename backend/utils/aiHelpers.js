      ...
      if (newQuestionMatch) {
        ...
        if (
          ...
          if (!currentQuestion.explanation)
            currentQuestion.explanation = `The correct answer is ${currentQuestion.correctAnswer.toUpperCase()}.`;
          ...
          if (correctAnswerText.match(/^[a-d]$/i)) {
            ...
            if (currentQuestion.answers.length === 0) {
              currentQuestion.answers.push({
                letter: 'true',
                answer: 'True',
                uniqueKey: `q${currentQuestion.id}-true`,
              });
              currentQuestion.answers.push({
                letter: 'false',
                answer: 'False',
                uniqueKey: `q${currentQuestion.id}-false`,
              });
            }
          } else {
            ...
            if (embeddedLetter) {
              ...
              console.warn(
                `Could not parse correct answer format: "${correctAnswerText}" for question ID ${currentQuestion.id}`,
              );
...
async function getExamContent(examId) {
  ...
  try {
    ...
    if (associatedModuleIds.length > 0) {
      console.log(
        `Workspaceing content from associated modules: ${associatedModuleIds.join(
          ', ',
        )}`,
      );
      const modulePromises = associatedModuleIds.map(modId =>
        db.collection('modules').doc(modId).get(),
      );
      ...
      moduleDocs.forEach(modDoc => {
        if (modDoc.exists) {
          ...
          content +=
            (modData.title || '') + ' ' + (modData.description || '') + ' ';
    ...
    if (content.trim() === '') {
      console.warn(
        `No exam-specific or associated module content found for examId: ${examId}. Falling back to ALL module content.`,
      );
      ...
      if (!modulesSnapshot.empty) {
        ...
        modulesSnapshot.forEach(doc => {
          content +=
            ...
            ' ';
          sectionPromises.push(
            db.collection('modules').doc(doc.id).collection('sections').get(),
          );
        });
        const allSections = await Promise.all(sectionPromises);
    ...
    if (content.trim() === '') {
      console.warn(
        'No content found for exam generation. Using default placeholder.',
      );
    ...
    console.log(
      `Final content context length for exam ${examId}: ${content.length} characters`,
    );
  } catch (error) {
