...
const DashboardScreen: FC<DashboardScreenProps> = ({navigation}) => {
  ...
  const fetchUserData = useCallback(
    async (currentProviderId: string, currentPathId: string) => {
      setLoading(true);
      ...
      try {
        ...
        if (!storedUserId) {
          console.error(
            '[DashboardScreen] User ID not found, cannot fetch data.',
          );
          setErrorInfo({
        ...
        console.log(
          `[DashboardScreen] Fetching data for user: ${storedUserId}, path: ${currentProviderId}/${currentPathId}`,
        );

        // Define response types inline for clarity
        interface ListModulesResponse {
          status: string;
          data: {modules: ApiModule[]; hasMore: boolean; lastId: string | null};
        }
        interface ListQuizzesResponse {
          status: string;
          data: {quizzes: ApiQuiz[]; hasMore: boolean; lastId: string | null};
        }
        interface ListExamsResponse {
          status: string;
          data: {exams: ApiExam[]; hasMore: boolean; lastId: string | null};
        }
        ...
        const results = await Promise.allSettled([
          ...
          axios.get<QuizHistoryResponse>(
            `${BASE_URL}/api/v1/quizzes/history/${storedUserId}`,
          ),
          // --- Call 6: Exam Results History ---
          axios.get<ExamHistoryResponse>(
            `${BASE_URL}/api/v1/exams/user/${storedUserId}/exam-history`,
          ),
        ]);
        ...
        if (results[0].status === 'fulfilled') {
          ...
          console.log(
            '[DashboardScreen] Progress Data Received:',
            progressResponse.data,
          );
          if (progressResponse.data.status === 'success') {
            fetchedProgress =
              progressResponse.data.data.overallProgress ||
              defaultOverallProgress;
            fetchedLearningPaths =
              progressResponse.data.data.learningPaths || [];
            ...
            console.error(
              'Error in progress response status:',
              progressResponse.data,
            );
            fetchError = {
        ...
        if (results[1].status === 'fulfilled') {
          ...
          console.log(
            '[DashboardScreen] Modules Data Received:',
          ...
          if (modulesResponse.data.status === 'success') {
            ...
            console.error(
              'Error in modules response status:',
              modulesResponse.data,
            );
            fetchError = {
              ...fetchError,
              message: `${
                fetchError?.message || ''
              } Failed to load quizzes.`.trim(),
              isIndexError: fetchError?.isIndexError || false,
            }; // Ensure isIndexError is boolean
          }
        } else {
        ...
        if (results[2].status === 'fulfilled') {
          ...
          console.log(
            '[DashboardScreen] Quizzes Data Received:',
          ...
          if (quizzesResponse.data.status === 'success') {
            ...
            console.error(
              'Error in quizzes response status:',
              quizzesResponse.data,
            );
            fetchError = {
              ...fetchError,
        ...
        if (results[3].status === 'fulfilled') {
          ...
          console.log(
            '[DashboardScreen] Exams Data Received:',
            examsResponse.data,
          );
          if (examsResponse.data.status === 'success') {
            ...
            console.error(
              'Error in exams response status:',
              examsResponse.data,
            );
            fetchError = {
              ...fetchError,
          ...
          fetchError = {
            ...
            message: `${
              fetchError?.message || ''
            } Failed to load quiz results.`.trim(),
            isIndexError: fetchError?.isIndexError || false,
          }; // Ensure isIndexError is boolean
        }
        ...
        if (results[4].status === 'fulfilled') {
          ...
          console.log(
            '[DashboardScreen] Quiz History Data Received:',
            quizHistoryResponse.data,
          );
          if (quizHistoryResponse.data.status === 'success') {
            // Correctly access the nested quizHistory array
            fetchedQuizResults =
              quizHistoryResponse.data.data.quizHistory || []; // Access data.quizHistory
            // --- ADD LOG: Show the actual fetched quiz results ---
            console.log(
              '[DashboardScreen] Fetched Quiz Results Array:',
              JSON.stringify(fetchedQuizResults, null, 2),
            );
          } else {
            console.error(
              'Error in quiz history response status:',
              quizHistoryResponse.data,
            );
            fetchError = {
              ...fetchError,
        ...
        if (results[5].status === 'fulfilled') {
          ...
          console.log(
            '[DashboardScreen] Exam History Data Received:',
            examHistoryResponse.data,
          );
          if (examHistoryResponse.data.status === 'success') {
            fetchedExamResults =
              examHistoryResponse.data.data?.examHistory || []; // Get results from dedicated endpoint
            console.log(
              '[DashboardScreen] Fetched Exam Results Array:',
              JSON.stringify(fetchedExamResults, null, 2),
            );
          } else {
            console.error(
              'Error in exam history response status:',
              examHistoryResponse.data,
            );
            fetchError = {
              ...fetchError,
              message: `${
                fetchError?.message || ''
              } Failed to load exam results.`.trim(),
              isIndexError: fetchError?.isIndexError || false,
            };
          }
        } else {
        ...
        setLoading(false);
        console.log(
          '[DashboardScreen] Fetching complete. Final state updates applied.',
        );
      }
    },
    [],
  ...
  useEffect(() => {
    ...
    console.log(
      '[DashboardScreen] Trying to determine active path from context.',
    );
    console.log('[DashboardScreen] Context Values:', {
      activeProviderId,
      activePathId,
    });
    ...
    if (activeProviderId && activePathId) {
      ...
      setErrorInfo({
        message: 'Could not determine the active learning path from context.',
        isIndexError: false,
      });
      setLoading(false);
    }
  ...
  const getResourceDetails = (
    ...
    if (resourceType === 'module') {
      ...
      if (item) {
        ...
        console.log(
          `[getResourceDetails] Processing exam: ID=${item.id}, Title=${item.title}. Attempting image lookup with key: '${item.id}'`,
        );
        title = item.title;
        ...
        console.log(
          `[getResourceDetails] Specific image found for key '${item.id}':`,
          imageIcon !== imageMapRecord['default'],
        ); // Log if specific image (not default) was found using item.id
      } else {
        console.log(
          `[getResourceDetails] Exam item not found for resourceId: ${resourceId}`,
        );
      }
    }
  ...
  const progressPercentage =
    totalAvailableModules > 0
      ? Math.round(
          (overallProgress.totalModulesCompleted / totalAvailableModules) * 100,
        )
      : 0;
  ...
  const examsById = useMemo(() => {
    console.log(
      '[DashboardScreen] useMemo calculating examsById. Input examResults:',
  ...
  console.log(
    '[DashboardScreen] Quizzes grouped by module:',
    JSON.stringify(quizzesByModule, null, 2),
  );
          ...
          <ErrorBanner
            ...
            onRetry={() => {
              ...
              if (activeProviderId && activePathId) {
                ...
                setErrorInfo({
                  message:
                    'Cannot retry: Learning path information is missing from context.',
                  isIndexError: false,
                });
              }
            }}
  ...
  const gridItems = [
    {
      ...
      description: 'Interactive concepts',
      color: '#3b82f6',
  ...
  return (
    <SafeAreaView
      style={[dashboardStyles.container, {backgroundColor: colors.background}]}>
      <ScrollView
        ...
        <View style={dashboardStyles.gridWrapper}>
          <Text style={[dashboardStyles.gridTitle, {color: colors.text}]}>
            Explore
          </Text>
        ...
        <Animated.View
          ...
          <Text style={[dashboardStyles.cardTitle, {color: colors.text}]}>
            Learning Progress
          </Text>
          ...
          <View style={dashboardStyles.progressSection}>
            <View style={dashboardStyles.progressLabelContainer}>
              <Text
                style={[dashboardStyles.progressLabel, {color: colors.text}]}>
                Overall Module Completion
              </Text>
              <Text
                style={[
                  dashboardStyles.progressPercentage,
            ...
            <View
              style={[
                dashboardStyles.progressBarContainer,
            ...
            <Text
              style={[
                dashboardStyles.progressSubText,
                {color: colors.textSecondary},
              ]}>
              {overallProgress.totalModulesCompleted} of {totalAvailableModules}{' '}
              modules completed.
            </Text>
          </View>
          ...
          {modules.length > 0 ? (
            modules.map(module => {
              const {imageIcon, color, title} = getResourceDetails(
                module.id,
                'module',
              );
              ...
              console.log(
                `[DashboardScreen] For Module "${title}" (ID: ${module.id}), passing ${moduleQuizzes.length} quizzes to QuizModule.`,
              );
          ...
          {quizzes.length > 0 ? (
            quizzes.map(quiz => {
              ...
              const latestResult = quizResults.find(
            ...
            <Text
              ...
              No quizzes available for this path.
            </Text>
          )}
          ...
          {exams.length > 0 ? (
            // --- Add this log ---
            (console.log(
              '[DashboardScreen] Rendering Exams List. Exams state:',
              exams,
            ),
            exams.map(exam => {
                  ...
                  status={
                    latestResult
                      ? `Latest: ${latestResult.percentage}% (${
                          latestResult.percentage ? 'Passed' : 'Failed'
                        })`
                      : status
                  } // Show latest result status
                  color={color}
                  imageIcon={imageIcon}
                  percentage={
                    latestResult
                      ? Math.round((latestResult.score / exam.totalScore) * 100)
                      : undefined
                  }
                  onPress={() =>
            ...
            <Text
              ...
              No exams available for this path.
            </Text>
          )}
        </Animated.View>
      </ScrollView>
